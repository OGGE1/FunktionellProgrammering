...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsInDatabase(){
        return (int) database.questions.stream().map(v -> v.question).count();
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
        return (int) database.questions.stream().filter(e -> e.getCategory().equals(category)).map(v -> v.answers)
                .count();
    }

    //Skapa en lista innehållandes samtliga frågesträngar i databasen
    public List<String> getListOfAllQuestions(){
        return database.questions.stream().map(e -> e.getQuestionString()).collect(Collectors.toList());
    }
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
        return database.questions.stream().filter(e -> e.getCategory()
                .equals(category)).map(v -> v.question)
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        return database.questions.stream().flatMap(e -> e.answers.stream()).distinct().collect(Collectors.toList());
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return database.getQuestions().stream().anyMatch(e -> e.getAllAnswers().contains(answerCandidate));
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return (int) database.getQuestions().stream().filter(e -> e.getAllAnswers().contains(answerCandidate)).count();
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory(){
        return database.getQuestions().stream()
                .collect(Collectors.groupingBy(Question::getCategory,Collectors.mapping(Question::getQuestionString,Collectors
                        .toList())));
    }
